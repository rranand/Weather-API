...
<html lang="en">
<head>
	<title>SignUp to Weather API</title>
	<meta charset="UTF-8">
...
<body style="background-color: #999999;">
	...
	<div class="limiter">
		<div class="container-login100">
			<div class="login100-more" style="background-image: url('images/signupbg.jpg');"></div>
...
<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $password = $_POST['pass'];
        $reppass = $_POST['repeat-pass'];

        $db=new database();

        $log = new login($db->Connect());
        $data = (boolean)$log->existance($username, $email);
        unset($_POST);

        if ($data==0 and $password==$reppass) {
            $log->create($name, $email, $username, $password);
            ?>
                <script>
            ...
            echo '<script>
                ...
                t.innerHTML = "Password do not match!!!";
            </script>';
            die();
        } else {
            echo '<script>
                var t = document.getElementById("onwrong");
                t.innerHTML = "Email or username already exist!!!";
            </script>';
            die();
        }
    }
